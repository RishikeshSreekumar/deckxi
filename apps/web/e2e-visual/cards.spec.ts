/**
 * Screenshot the card gallery — sizes/states, the full-edition grid, and the
 * share/og composition. Any change to tokens, the TrumpCard or the card back
...
// The fictional fixture edition: the baselines must not move when the real
// deck is refreshed from source data.
const EDITION_ID = "edition-fixture";
const edition = require(`@deckxi/data/editions/${EDITION_ID}.json`) as {
  players: { id: string }[];
};

test.beforeEach(async ({ page }) => {
  await page.goto(`/cards?edition=${EDITION_ID}`);
...
test("share composition", async ({ page }) => {
  ...
  await page.goto(`/cards/share/${first?.id ?? ""}?edition=${EDITION_ID}`);
