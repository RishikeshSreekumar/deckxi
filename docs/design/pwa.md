...
Offline, the shell and the card art come out of the cache, so the app opens and every
screen outside a room still reads — but a game is a room on the server, so there is
nothing to play until the network is back. (An on-device game against bots, #85, rode on
this worker for a while and has since been removed: the product is people playing each
other.)
